...
async def test_get_recent_commits_encodes_ref_and_limit(monkeypatch):
    ...
    result = await github_client.get_recent_commits(
        "example",
        "project",
        ref="feature/api",
        limit=25,
...
async def test_get_repository_paths_filters_non_files(monkeypatch):
    async def fake_get(path: str):
    ...
    paths, truncated = await github_client.get_repository_paths(
        "example",
        "project",
        "main",
    )
